package com.teachmall.orders.api;
...
import com.teachmall.orders.util.SecurityUtil;
...
public class OrderController {
    ...
    public PayRecordDto generatePayCode(@RequestBody AddOrderDto addOrderDto) {
        ...
        if(user == null){
            TeachmallException.cast("请登录后继续选课");
        }
        return orderService.createOrder(user.getId(), addOrderDto);

    }
    @ApiOperation("购买优惠券")
    @PostMapping("/coupon/purchase/{couponId}")
    @ResponseBody
    public void purchaseCoupon(@PathVariable("couponId") Long couponId) {
    ...
    public void requestpay(String payNo,HttpServletResponse httpResponse) throws IOException {
        ...
        if("601002".equals(status)){
            TeachmallException.cast("订单已支付，请勿重复支付。");
        }
        //构造sdk的客户端对象
...
//        alipayRequest.setReturnUrl("http://domain.com/CallBack/return_url.jsp");
        alipayRequest.setNotifyUrl(AlipayConfig.notify_url+"receivenotify");//在公共参数中设置回跳和通知地址
